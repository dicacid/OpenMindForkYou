...
3) Generate gateway token and write `~/.openmind/moltbot.json`:
...
- POST `/api/molt/start` { provider: "anthropic"|"openai", apiKey: string }
  ...
  - Generate token; write/update `~/.openmind/moltbot.json` with basePath `/api/moltbot` and auth token
...
- ✅ Installed Moltbot (openmind) 2026.1.24-3 via official installer
...
- ✅ Created installation script that installs Node.js and openmind to /root (persisted path)
- ✅ Node.js installed to /root/nodejs
- ✅ OpenMind installed to /root/.openmind-bin with symlink
- ✅ Created wrapper script /root/run_openmind.sh for reliable execution
